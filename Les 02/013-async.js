...
var loadArrayAsync = function (arrayA , elements, cb) {
    ...
    //forEach is slower?
    //forEach returnt niet de index maar het element
    //for en forEach is even snel ongeveer buiten als forEach negatief telt
        ...
        loadAsync(element, function (err, element) {
            if (err) {
                ...
                if (++index === usersIds.length){
                    cb(null); //cb(null,null); //cb(arrayA);
                }
            }
        });
    });
